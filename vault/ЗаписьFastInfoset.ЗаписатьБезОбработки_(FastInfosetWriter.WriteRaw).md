...
В случае, когда используется запись в Fast Infoset-файл, метод используется как аналог метода [ЗаписьFastInfoset.ЗаписатьТекст](ЗаписьFastInfoset.ЗаписатьТекст_(FastInfosetWriter.WriteText).md). Разбор и анализ записываемых текстовых данных не производится.  
