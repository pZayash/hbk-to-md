...
<!-- signature:start -->
`ОткрытьПоток`(`<Поток>`: [`Поток`](Поток.md) | [`ПотокВПамяти`](ПотокВПамяти.md) | [`ФайловыйПоток`](ФайловыйПоток.md), `<Параметры>`: [`ПараметрыЧтенияXML`](ПараметрыЧтенияXML.md), `<НаборСхемXML>`: [`НаборСхемXML`](НаборСхемXML.md), `<Кодировка>`: `Строка` | [`КодировкаТекста`](КодировкаТекста.md))
<!-- signature:end -->
...
ОткрытьПоток(<Поток>, <Параметры>, <НаборСхемXML>, <Кодировка>)
...
Поток для чтения XML-данных.

<Параметры> (необязательный)

Тип: [ПараметрыЧтенияXML](ПараметрыЧтенияXML.md).   
Параметры чтения, которые будут использованы при чтении данных XML.
...
Тип: Строка, [КодировкаТекста](КодировкаТекста.md).   
Позволяет задать кодировку входного потока. Следует указывать для текстов в кодировках:
...
Открывает поток для чтения данным объектом. Если перед вызовом данного метода уже выполнялось чтение XML из другого файла или строки, то чтение прекращается и объект инициализируется для чтения из указанного потока.
