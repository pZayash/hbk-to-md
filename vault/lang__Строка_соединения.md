...
**Строка соединения**— [строка](lang__def_String.md), задающая параметры информационной базы, каждый из которых представляет собой фрагмент вида <Имя параметра>=<Значение>, где:
...
Строка соединения указывается в форме "Запуск 1С:Предприятия" под списком информационных баз, может быть задана в [параметре командной строки выбора режима](v8help:__1cv8__ZIF1.md) CREATEINFOBASE, как параметр метода **СоздатьНачальныйОбраз()**.
