...
CLI-конвертер `.hbk` (1С) → плоский каталог `.md` для Obsidian / `qmd`-индексации / ИИ-агентов.

## Установка

```bash
python -m pip install -r tools/hbk-to-md/requirements.txt
```
...
```bash
python -m pytest tools/hbk-to-md/tests/
```
